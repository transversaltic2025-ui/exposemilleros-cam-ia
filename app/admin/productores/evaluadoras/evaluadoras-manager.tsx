...
export function EvaluadorasManager() {
  ...
  const load = useCallback(async () => {
    ...
    else setError(String(data?.error || "No fue posible consultar los evaluadores."));
  }, []);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (requestPayload.nombre.length < 3) { setError("Ingrese el nombre completo del evaluador."); return; }
    ...
    try {
      ...
      const data = await readPayload(response);
      if (!response.ok) { setError(String(data?.error || "No fue posible registrar el evaluador.")); return; }
      setForm({ nombre: "", documento: "", correo: "", activo: true });
      setSuccess("Evaluador registrado correctamente.");
      await load();
    } catch (requestError) {
      console.error("[evaluadoras] error de solicitud", requestError);
      setError("No fue posible registrar el evaluador.");
  ...
  async function update(item: EvaluadoraProductores, changes: Partial<EvaluadoraProductores>) {
    ...
    const data = await readPayload(response);
    if (!response.ok) { setError(String(data?.error || "No fue posible actualizar el evaluador.")); return; }
    await load();
  }
  ...
  return <div className="mt-8 grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
    <Card><CardHeader><CardTitle>Registrar evaluador</CardTitle></CardHeader><CardContent>
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <div><Label>Estado</Label><select className="mt-2 h-11 w-full rounded-xl border bg-white px-3" value={form.activo ? "activo" : "inactivo"} onChange={event => setForm({ ...form, activo: event.target.value === "activo" })}><option value="activo">Activo</option><option value="inactivo">Inactivo</option></select></div>
        {error && <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}
        {success && <p className="rounded-xl bg-green-50 p-3 text-sm text-green-700">{success}</p>}
        <Button type="submit" disabled={busy}>{busy ? "Registrando..." : "Registrar evaluador"}</Button>
      </form>
    </CardContent></Card>
    <Card><CardHeader><CardTitle>Evaluadores registrados</CardTitle></CardHeader><CardContent className="space-y-3">
      {items.map(item => <div key={item.id} className="rounded-xl border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-bold">{item.nombre}</p><p className="text-sm text-[var(--color-muted)]">{item.documento} · {item.correo || "Sin correo"}</p><p className="text-xs text-[var(--color-muted)]">{item.activo ? "Activo" : "Inactivo"} · {item.created_at ? new Date(item.created_at).toLocaleDateString("es-CO") : ""}</p></div><div className="flex gap-2"><Button type="button" variant="outline" onClick={() => edit(item)}>Editar</Button><Button type="button" variant="outline" onClick={() => update(item, { activo: !item.activo })}>{item.activo ? "Desactivar" : "Activar"}</Button></div></div></div>)}
    </CardContent></Card>
  </div>;
}
