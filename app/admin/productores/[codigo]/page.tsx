...
export default async function Page({ params }: { params: Promise<{ codigo: string }> }) {
  ...
  return <SiteShell>
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-2">
      ...
      <Card className="lg:col-span-2"><CardHeader><CardTitle>Resumen de resultados</CardTitle></CardHeader><CardContent><p className="mb-5 text-sm leading-6 text-[var(--color-muted)]">La evaluación humana es el criterio oficial de los evaluadores. El análisis IA funciona como apoyo complementario para la toma de decisiones.</p>{!humanAvailable && !aiAvailable ? <p>Aún no hay resultados registrados para esta iniciativa.</p> : <div className="grid gap-4 sm:grid-cols-2"><div className="rounded-xl border p-4"><p className="font-bold">Evaluación humana</p>{humanAvailable ? <><p className="mt-2"><b>Promedio:</b> {humanPercentage}%</p><p><b>Nivel de tendencia:</b> {humanLevel}</p></> : <p className="mt-2 text-[var(--color-muted)]">Evaluación humana pendiente.</p>}</div><div className="rounded-xl border p-4"><p className="font-bold">Análisis IA</p>{aiAvailable ? <><p className="mt-2"><b>Porcentaje IA:</b> {analysis.porcentaje_ia}%</p><p><b>Nivel de tendencia IA:</b> {analysis.nivel_tendencia_ia}</p></> : <p className="mt-2 text-[var(--color-muted)]">Análisis IA pendiente.</p>}</div></div>}</CardContent></Card>

      <Card><CardHeader><CardTitle>Evaluación humana de evaluadores</CardTitle><p className="text-sm leading-6 text-[var(--color-muted)]">Resultado registrado por los evaluadores autorizados. Esta evaluación corresponde al criterio humano del equipo evaluador.</p></CardHeader><CardContent className="space-y-4">{evaluations?.map(evaluation => <div key={evaluation.id} className="space-y-2 rounded-xl border p-4"><p className="font-bold">Evaluación de {evaluation.evaluadoras_productores?.nombre_completo || "evaluador autorizado"}</p><p><b>Porcentaje:</b> {evaluation.porcentaje ?? 0}%</p><p><b>Nivel de tendencia:</b> {visible(evaluation.nivel_tendencia)}</p><p><b>Concepto del evaluador:</b> {visible(evaluation.concepto_evaluadora)}</p><p><b>Fortalezas:</b> {visible(evaluation.fortalezas)}</p><p><b>Aspectos por mejorar:</b> {visible(evaluation.aspectos_mejora)}</p><p><b>Apoyo recomendado:</b> {visible(evaluation.apoyo_recomendado)}</p></div>)}{!humanAvailable && <p>Sin evaluaciones todavía.</p>}</CardContent></Card>
